...
App.prototype.writeFile = function(file, data)
{
    return this.cep.fs.writeFile(file, data);
};
 ...
 * @param callback
 */
App.prototype.request = function(url, param, callback)
{
    var p = "";
    for (var k in param) {
        p += k + '=' + param[k] + '&';
...
App.prototype.showToast = function(text, timeout) {
    timeout = (timeout == undefined)? 3000 : time;
    $('#toast').html('<span>' + text + '</span>').show().animate({bottom: '60px'}, 'fast', 'swing', function() {
        setTimeout(function() {
