...
App.prototype.themeChange = function()
{
    ...
    var theme = {
        '#343434': ['dark', './css/dark.css?jmmjjjmm', './css/widget-dark.css?jjmm'],
        '#535353': ['dark', './css/dark.css?jmmjjjmm', './css/widget-dark.css?jjmm'],
        '#b8b8b8': ['gray', './css/gray.css?jmmj', './css/widget-gray.css?jj'],
        '#d6d6d6': ['gray', './css/gray.css?jmmj', './css/widget-gray.css?jj']
    };
    if (theme[colorRGB] != undefined) {
        $('body').removeClass('dark').removeClass('gray').addClass(theme[colorRGB][0]);
        $('#theme-css').attr('href', theme[colorRGB][1]);
        $('#theme-widget-css').attr('href', theme[colorRGB][2]);
    }
...
App.prototype.showToast = function(text, timeout) {
    ...
    $('#toast').html('<span>' + text + '</span>').show().animate({bottom: '40px'}, 'fast', 'swing', function() {
        setTimeout(function() {
            $('#toast').animate({bottom: '-40px'}, 'fast', 'swing', function() {
                $(this).hide();
            })
        }, timeout);
    });
